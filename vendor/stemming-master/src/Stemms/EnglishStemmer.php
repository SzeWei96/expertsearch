    ...
    */
    private static function step1ab($word)
    {
        ...
        if (substr($word, -2, 1) != 'e' or !self::replace($word, 'eed', 'ee', 0)) { // First rule
            ...
            // ing and ed
            if (preg_match("#$v+#", substr($word, 0, -5)) && self::replace($word, 'gging', 'g')
		or preg_match("#$v+#", substr($word, 0, -3)) && self::replace($word,'ing', 'ing')
        or preg_match("#$v+#", substr($word, 0, -3)) && self::replace($word, 'ied', 'y')
		or preg_match("#$v+#", substr($word, 0, -3)) && self::replace($word, 'ved', 've')
    ...
    */
    private static function step4($word)
    {
        switch (substr($word, -2, 1)) {
            ...
            case 'c':
                   self::replace($word, 'vance', 'vant', 1)
                or self::replace($word, 'ance', '', 1)
